...
fixPort() {  
	...
	kill -9 $(lsof -i :$PORT | awk '{print $2}' | grep -o '[0-9]\+')
...
newMachineSetup() {
	...
	echo "FreshInstall, Unpack"; read answer
	...
	if [ "$answer" == "FreshInstall" ]; then
		...
		apt-get install dos2unix nginx sudo zip unzip lsof default-jre screen htop build-essential 
		apt-get install nodejs cpufrequtils figlet
		...
		## IF THERE ARE ANY ISSUES AND YOU NEED JAVA 8,
...
adminPanelMenu() {
	...
	while true; do
		...
		if [ "$admininput" == "RFWall" ]; then resetFirewall ; fi;
